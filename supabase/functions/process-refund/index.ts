...
serve(async (req) => {
  ...
  try {
    ...
    // refund_type: 'partial' (₦4,700) | 'full' (₦5,000) | 'wallet_credit' (₦5,000 credit) | 'none' | 'manual' (keep open for admin manual resolution)
    ...
    // Manual resolution: mark as open pending manual closure — don't refund or cancel job yet
    ...
    if (refund_type === 'wallet_credit') {
      ...
      // Cancel job and mark as dispute-resolved
      if (dispute.job_id) {
        await supabase
          .from('jobs')
          .update({ status: 'cancelled', cancellation_reason: `Dispute resolved: Wallet credit refund approved by admin. Reason: ${dispute.reason}` })
      ...
      // Close dispute without refund — mark job as dispute-resolved if it has one
      if (dispute.job_id) {
        await supabase
          .from('jobs')
      ...
      if (dispute.job_id) {
        await supabase
          .from('jobs')
          .update({ status: 'cancelled', cancellation_reason: `Dispute resolved: Cash refund approved by admin. Reason: ${dispute.reason}` })
          .eq('id', dispute.job_id);
      }
    }

    // Resolve and close the dispute (for all non-manual options)
